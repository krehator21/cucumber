...
public class Stepdefs {
    ...
    //(Larry|Lucy) pipes specifies the alternative values to use here
    @Then("^(Larry|Lucy) does not hear Sean's message$")
    public void listerner_does_not_hear_Sean_s_message(String listenerName) throws Throwable {
        List<String> heardByListener = people.get(listenerName).getMessagesHeard();
        List<String> messagesFromSean = messagesShoutedBy.get("Sean");
        String[] messagesFromSeanArray = messagesFromSean.toArray(new String[messagesFromSean.size()]);
